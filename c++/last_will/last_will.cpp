...
namespace estate_executor {
    int assemble_account_number(int secret_modifier) {
        // sum the families bank number parts with the secret modifier
        int secret_account_number{ zhang::bank_number_part(secret_modifier) + khan::bank_number_part(secret_modifier) + garcia::bank_number_part(secret_modifier) };
        return secret_account_number;
    }
    int assemble_code() {
        // sum the red fragments and blue fragments seperately, then multiply the totals
